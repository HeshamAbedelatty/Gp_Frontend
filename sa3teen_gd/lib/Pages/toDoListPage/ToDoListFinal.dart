...
class _ToDoListScreenState extends State<ToDoListScreen> {
  List<ToDoListClass> toDoLists = [];
    List<TextEditingController> listNameControllers = []; // List of controllers for list names
...
toDoLists.add(ToDoListClass(listName: "", tasks: []));
      listNameControllers.add(TextEditingController()); // Add a new controller for the new list
      // toDoLists.add(ToDoListClass(listName: "", tasks: []));
    });
  }
  ...
  void editTask(
      int listIndex, int taskIndex) {
    setState(() {
      toDoLists[listIndex].tasks[taskIndex]=Task(taskName: taskNameController.text,deadline: DateTime.parse(dateController.text));
    });
  }
  ...
  Task? searchTask(String keyword) {
    ...
    for (var list in toDoLists) {
      for (var task in list.tasks) {
        ...
        else{print('No tasks found matching the search term.');}
      }
    }
    return null;
  }
                        ...
                        children: [
                          Expanded(
                            child: TextFormField(
                              ...
                              controller: listNameControllers[listIndex], // Use the corresponding controller,
                              onChanged: (value) {
                                setState(() {
